...
class GreeterServicer(greet_pb2_grpc.GreeterServicer):
    ...
    def ParrotSaysHello(self, request, context):
        ...
        for i in range(3):
            ...
            hello_reply = greet_pb2.HelloReply(**payload)
            yield hello_reply
            time.sleep(3)
    ...
    def ChattyClientSaysHello(self, request_iterator, context):
        ...
        delayed_reply.message = f"You sent {len(delayed_reply.request)} messages"
    ...
    def InteractingHello(self, request_iterator, context):
        for request in request_iterator:
            ...
            print(request)
            payload = {
                "message": f"Server: {request.name} Said {request.greeting}"
            }
            hello_reply = greet_pb2.HelloReply(**payload)
            yield hello_reply
